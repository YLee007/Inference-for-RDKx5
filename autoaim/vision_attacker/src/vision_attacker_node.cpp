...
double solveTrajectory(double &flyTime, const double x, const double z, 
                      const double lagTime, const double muzzleSpeed, const double airK) 
{
    ...
    for (int i = 0; i < maxIterations; ++i) {
        ...
        // 水平时间计算（含阻力）
        double denominator = airK * muzzleSpeed * cosTheta;
        double kxTerm = airK * x;
        if (denominator <= 0 || kxTerm >= denominator) break;
        time = -log(1 - kxTerm / denominator) / airK;

        // 垂直位移（仅重力模型）
        double realZ = muzzleSpeed * sin(theta) * time - 0.5 * gravity * pow(time, 2);

        if (abs(realZ - z) < tolerance) {
            converged = true;
            break;
        }

        // 牛顿法导数（简化版）
        double derivative = (muzzleSpeed * cosTheta * time) / (1 - kxTerm/denominator);
        if (abs(derivative) > 1e-6) {
            theta -= (realZ - z) / derivative;
        }
        
        theta = std::clamp(theta, -M_PI/4, M_PI/4); // 限制在±45°内
    }

    // 滞后时间补偿（仅重力影响）
    flyTime = time + lagTime;
    double finalZ = z + 0.5 * gravity * pow(lagTime, 2);
    
    return atan2(finalZ, x);
}
