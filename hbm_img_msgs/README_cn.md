...
支持在X5 Ubuntu系统上编译和在PC上使用docker交叉编译两种方式，并支持通过编译选项控制编译pkg的依赖和pkg的功能。

### X5 Ubuntu系统上编译
1、编译环境确认

- 当前编译终端已设置ROS环境变量：`source /opt/ros/humble/setup.bash`。
...
编译成功后，将生成的install路径拷贝到RDK X5开发板上（如果是在X5上编译，忽略拷贝步骤），并执行如下命令运行
